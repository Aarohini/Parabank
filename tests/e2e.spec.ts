...
test.describe('TC-E2E-01: Validate New Account Created via UI Exists in API Response', () => {
    ...
    test(
      'TC-E2E-01: Should register user, open account via UI, and verify account ID via API',
      async ({ page, apiContext }) => {

        const scenarios = getTestScenarios();
//using the valid registraton scenario using expectSuccess field
        const scenario = scenarios.find(scenario => scenario.expectSuccess === true);

        if (!scenario) {
          throw new Error('No valid successful registration scenario found');
        }
...
//creating new account via ui
        ...
        await expect(openAccountPage.accountOpenedMessage).toBeVisible();
...
//validation via api
        ...
        expect(responseText).toContain(
          `<id>${uiAccountNumber}</id>`
        );



        console.log(`Account verified successfully. UI Account ID (${uiAccountNumber}) matches API response.`);
